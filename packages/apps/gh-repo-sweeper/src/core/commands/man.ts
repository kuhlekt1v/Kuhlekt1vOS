...
import type { ExecuteParams } from "../command";
const MARGIN = 5;
...
export const man = new Command()
  ...
  .setManual({
    ...
    usage: "man [command] page\n" + "man -k [] regexp",
    description:
      "Each page arguments given to man is normally the name of a command.\n" +
  ...
  .setExecute(function (this: Command, args, params) {
    ...
    if (options?.includes("k")) {
      const commands = CommandsManager.search(
        (args as string[])[0].toLowerCase()
      );
      return commands
        .map((command) => {
    ...
    if (!manual) return formatError(`${commandName}: No manual found`);
    ...
    if (manual.purpose) {
      sections[0].push(
        formatText(
          `${commandName} - ${ANSI.decoration.dim}${ANSI.fg.yellow}${manual.purpose}${ANSI.reset}`
        )
      );
    } else {
    ...
    if (manual.usage) {
      sections.push(["SYNOPSIS", formatText(manual.usage)]);
    }

    if (manual.description) {
      sections.push(["DESCRIPTION", formatText(manual.description)]);
    }
    ...
    if (manual.prerequisites) {
      sections.push(["PREREQUISITES", formatText(manual.prerequisites)]);
    }
